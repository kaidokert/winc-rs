...
use wincwifi::{Handle, ClientSocketOp};
...
impl EventListener for SocketCallbacks {
    ...
    fn on_connect(&mut self, socket: Socket, err: SocketError) {
        if let Some((s,op)) = self.tcp_sockets.get(Handle(socket.v)) {
...
impl From<embedded_nal::nb::Error<StackError>> for StackError {
    fn from(inner: embedded_nal::nb::Error<StackError>) -> Self {
        match inner {
            embedded_nal::nb::Error::WouldBlock => StackError::MyWouldBlock,
...
impl<'a, X: wincwifi::transfer::Xfer, E: EventListener> UdpClientStack for WincClient<'a, X, E> {
    ...
    fn send(&mut self, socket: &mut Self::UdpSocket, buffer: &[u8]) -> feather::hal::prelude::nb::Result<(), Self::Error> {
        todo!()
    }
    ...
    fn receive(
            ...
            socket: &mut Self::UdpSocket,
            buffer: &mut [u8],
        ) -> feather::hal::prelude::nb::Result<(usize, embedded_nal::SocketAddr), Self::Error> {
        todo!()
    }
