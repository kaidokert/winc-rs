...
use crate::ops::module::{PrngOp, ProvisioningMode, StationMode, SyncOp};
...
impl<X: Xfer> AsyncClient<'_, X> {
    ...
    pub async fn get_random_bytes(&mut self, buf: &mut [u8]) -> Result<(), StackError> {
        let mut op = PrngOp::new(buf);
        self.poll_op(&mut op).await
    }
}
...
mod tests {
    ...
    #[apply(test!)]
    async fn test_async_prng_timeout() {
        let mut client = make_test_client();
        let mut random_bytes = [0x0u8; 32];

        let result = client.get_random_bytes(&mut random_bytes).await;
        assert_eq!(result, Err(StackError::GeneralTimeout));
    }

    #[apply(test!)]
    async fn test_async_prng_success() {
        ...
        let result = {
            let mut client = make_test_client();
            *client.debug_callback.borrow_mut() = Some(&mut my_debug);
            client.get_random_bytes(&mut random_bytes).await
        };
    ...
    #[apply(test!)]
    async fn test_async_prng_large_buffer() {
