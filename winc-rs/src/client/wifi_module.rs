...
impl<X: Xfer> WincClient<'_, X> {
    ...
    /// # Arguments
    ///
    /// * `data` - Data buffer to store random bytes generated by the Wi-Fi chip.
    ///            The number of random bytes generated by the chip will depend on
    ///            the length of the data buffer.
    ///
    /// # Return
    ///
    /// * `()` - If random bytes are generated.
    /// * `StackError` - Error occurred during random byte generation.
    pub fn get_random_bytes(&mut self, data: &[u8]) -> nb::Result<(), StackError> {
        match &mut self.callbacks.prng_event_received {
            None => {
                self.manager
                    .send_prng(&data as *const _ as u32, data.len() as u16)
                    .map_err(StackError::WincWifiFail)?;
                self.callbacks.prng_event_received = Some(None);
            }
            Some(_result) => {
                if let Some(true) = _result.take() {
                    self.callbacks.prng_event_received = None;
                    return Ok(());
                }
            }
        }
