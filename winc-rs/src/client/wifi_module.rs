...
const REQUEST_TIMEOUT_MILLISECONDS: u32 = 5_000;
...
impl<X: Xfer> WincClient<'_, X> {
    ...
    pub fn disconnect_ap(&mut self) -> nb::Result<(), StackError> {
        match &mut self.callbacks.state {
            WifiModuleState::ConnectedToAp => {
                self.operation_countdown = REQUEST_TIMEOUT_MILLISECONDS;
    ...
    /// Gets random bytes using the pseudo-random number generator (PRNG).
    ///
    /// # Arguments
    ///
    /// * `data` - A data buffer to store the random bytes generated by the Wi-Fi chip.
    ...
    /// # Returns
    ///
    /// * `()` - If random bytes are successfully generated.
    ...
    pub fn get_random_bytes(&mut self, data: &mut [u8]) -> nb::Result<(), StackError> {
        match &mut self.callbacks.prng_buffer {
            None => {
                self.operation_countdown = REQUEST_TIMEOUT_MILLISECONDS;
                self.manager
                    .send_prng(&data as *const _ as u32, data.len() as u16)
                    .map_err(StackError::WincWifiFail)?;
                self.callbacks.prng_buffer = Some(None);
            }
            Some(prng) => {
                if let Some(buffer) = prng.take() {
                    data.copy_from_slice(&buffer[0..data.len()]);
                    self.callbacks.prng_buffer = None;
                    return Ok(());
                } else {
...
mod tests {
    ...
    use crate::manager::{Ssid, PRNG_DATA_LENGTH};
    ...
    #[test]
    fn test_prng_buffer_overflow() {
        let mut client = make_test_client();
        let mut random_bytes = [0x0u8; PRNG_DATA_LENGTH + 1];

        let result = nb::block!(client.get_random_bytes(&mut random_bytes));
        assert_eq!(result, Err(StackError::WincWifiFail(Error::BufferError)));
    }
}
